...
export default function SoppGenerator({ type, setView }) {
  ...
  const [pajakAda, setPajakAda] = useState(true);
  ...
  useEffect(() => {
    ...
    setRows(prev => {
      ...
      const r1 = prev[0] || { id: 1, kode: defaultR1Kode, uraian: defaultR1Uraian, debet: "", kredit: "" };
      const r2 = prev[1] || { id: 2, kode: "214.02.02", uraian: defaultR2Uraian, debet: "", kredit: "" };
      const r3 = prev[2] || { id: 3, kode: defaultR3Kode, uraian: defaultR3Uraian, debet: "", kredit: "" };
      const r4 = prev[3] || { id: 4, kode: "112.01.03", uraian: "Bank BRI", debet: "", kredit: "" };
  ...
  }, [dasarPengenaan, nilaiPajak, jumlah, pajakAda, pphPasal, dibayarkanKepada]);
  ...
  const scrollToPreviewField = (selectorId) => {
    const container = document.getElementById("sopp-preview-scroll-container");
  ...
  const handlePrint = () => {
    document.body.classList.add("print-sopp-only");
    window.print();
    setTimeout(() => {
      document.body.classList.remove("print-sopp-only");
    }, 450);
  };

  // ---------------------------------------------------------------------
  // STYLE HELPERS (koordinat exact dari PDF, dalam satuan pt)
  // ---------------------------------------------------------------------
  ...
  const sigCols = [
    ...
    { center: 512.7, label: "Disetujui (otorisator)", nama: disetujuiNama, jabatan: disetujuiJabatan }
  ];
  const SIG_COL_WIDTH = 170;

  const totalDebet = rows.reduce((sum, r) => sum + (parseFloat(r.debet) || 0), 0);
    ...
    <div className="max-w-7xl mx-auto mt-6 print:hidden">
      ...
      <style>{`
        @media print {
          ...
          body.print-sopp-only * { visibility: hidden !important; }
          body.print-sopp-only #sopp-print-area,
          body.print-sopp-only #sopp-print-area * { visibility: visible !important; color: #000000 !important; }
          body.print-sopp-only #sopp-print-area {
      ...
      {/* HEADER BAR */}
      <div className="bg-white dark:bg-[#0c1410] rounded-2xl shadow-sm border border-gray-100 dark:border-[#213527] mb-6 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-white leading-tight">
              SOPP - Otorisasi Pembayaran (Pengadaan)
            </h2>
            <p className="text-xs text-gray-400 dark:text-slate-400 mt-0.5">
              Template presisi 1:1 dengan dokumen cetak asli (Letter size).
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2 border border-gray-200 dark:border-[#2b4533] text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-[#1a2b20] rounded-xl text-sm font-semibold transition-all"
          >
            <ArrowLeft className="w-4 h-4" /> Kembali
          </button>
          <button
            onClick={handlePrint}
            className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        {/* ================================================================= */}
        {/* INPUT EDITOR (LEFT PANEL) - tidak diubah, hanya form input        */}
        {/* ================================================================= */}
        <div className="xl:col-span-5 space-y-6 max-h-[85vh] overflow-y-auto pr-1 custom-scrollbar">
          <div className="bg-white dark:bg-[#0c1410] rounded-2xl border border-gray-100 dark:border-[#213527] p-6 space-y-4 shadow-2xs">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Nomor Urut</label>
                <input type="text" value={nomorUrut} onChange={(e) => setNomorUrut(e.target.value)} onFocus={() => scrollToPreviewField("pv-nomor-urut")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500 font-mono" />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Tanggal Dokumen</label>
                <input type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)} onFocus={() => scrollToPreviewField("pv-tanggal")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
              </div>
            </div>
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Nama Unit Kerja / Divisi</label>
              <input type="text" value={unitKerja} onChange={(e) => setUnitKerja(e.target.value)} onFocus={() => scrollToPreviewField("pv-unit-kerja")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#0c1410] rounded-2xl border border-gray-100 dark:border-[#213527] p-6 space-y-4 shadow-2xs">
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-gray-50 dark:border-[#213527] pt-3">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Nomor Rekening</label>
                <input type="text" value={noRekening} onChange={(e) => setNoRekening(e.target.value)} onFocus={() => scrollToPreviewField("pv-nomor-rekening")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Atas Nama Rekening</label>
                <input type="text" value={atasNama} onChange={(e) => setAtasNama(e.target.value)} onFocus={() => scrollToPreviewField("pv-nomor-rekening")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Nama Bank</label>
                <input type="text" value={namaBank} onChange={(e) => setNamaBank(e.target.value)} onFocus={() => scrollToPreviewField("pv-nama-bank")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Cabang Bank</label>
                <input type="text" value={cabang} onChange={(e) => setCabang(e.target.value)} onFocus={() => scrollToPreviewField("pv-cabang")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
              </div>
            </div>
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">NPWP (Bila Ada)</label>
              <input type="text" value={npwp} onChange={(e) => setNpwp(e.target.value)} onFocus={() => scrollToPreviewField("pv-npwp")} className="w-full px-3 py-2 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3] focus:border-emerald-500" />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#0c1410] rounded-2xl border border-gray-100 dark:border-[#213527] p-6 space-y-4 shadow-2xs">
            ...
            <div className="flex gap-4 items-center">
              ...
              <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-slate-400 cursor-pointer">
                <input type="checkbox" checked={pajakAda} onChange={() => setPajakAda(true)} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="rounded text-emerald-600 focus:ring-emerald-500" /> Ada
              </label>
              <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-slate-400 cursor-pointer">
                <input type="checkbox" checked={!pajakAda} onChange={() => setPajakAda(false)} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="rounded text-emerald-600 focus:ring-emerald-500" /> Tidak ada
              </label>
            </div>
            {pajakAda && (
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Dasar Pengenaan (Rp)</label>
                  <input type="text" value={formatRibuan(dasarPengenaan)} onChange={(e) => setDasarPengenaan(parseRibuan(e.target.value))} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="w-full px-2 py-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3]" />
                </div>
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">PPh Pasal</label>
                  <input type="text" value={pphPasal} onChange={(e) => setPphPasal(e.target.value)} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="w-full px-2 py-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3]" />
                </div>
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Tarif (%)</label>
                  <input type="text" value={tarif} onChange={(e) => setTarif(e.target.value)} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="w-full px-2 py-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3]" />
                </div>
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 dark:text-slate-400 uppercase mb-1">Nilai Pajak (Rp)</label>
                  <input type="text" value={formatRibuan(nilaiPajak)} onChange={(e) => setNilaiPajak(parseRibuan(e.target.value))} onFocus={() => scrollToPreviewField("pv-dasar-pengenaan")} className="w-full px-2 py-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3]" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-[#0c1410] rounded-2xl border border-gray-100 dark:border-[#213527] p-6 space-y-4 shadow-2xs">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#213527] pb-2">
              <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-400">Rincian Perkiraan</h3>
              <button onClick={addRow} className="flex items-center gap-1 text-[11px] font-bold text-emerald-600 hover:text-emerald-700 hover:underline">
            ...
            <div className="space-y-3">
              {rows.map((row, index) => (
                <div key={row.id} className="border border-gray-100 dark:border-[#213527] rounded-xl p-3 space-y-2 relative bg-gray-50/50 dark:bg-[#0f1712]/30">
                  <button onClick={() => removeRow(row.id)} disabled={rows.length <= 1} className="absolute right-3 top-3 text-red-500 hover:text-red-700 disabled:opacity-30 cursor-pointer">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="text" value={row.kode} onChange={(e) => handleRowChange(row.id, "kode", e.target.value)} onFocus={() => scrollToPreviewField("pv-rincian-perkiraan")} className="w-full p-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-md text-gray-800 dark:text-[#f1f5f3] font-mono" />
                    </div>
                    <div>
                      <label className="text-[9px] font-semibold text-gray-400 uppercase">Uraian</label>
                      <input type="text" value={row.uraian} onChange={(e) => handleRowChange(row.id, "uraian", e.target.value)} onFocus={() => scrollToPreviewField("pv-rincian-perkiraan")} className="w-full p-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-md text-gray-800 dark:text-[#f1f5f3]" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[9px] font-semibold text-gray-400 uppercase">Debet (Rp)</label>
                      <input type="text" value={formatRibuan(row.debet)} onChange={(e) => handleRowChange(row.id, "debet", parseRibuan(e.target.value))} onFocus={() => scrollToPreviewField("pv-rincian-perkiraan")} className="w-full p-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-md text-gray-800 dark:text-[#f1f5f3]" />
                    </div>
                    <div>
                      <label className="text-[9px] font-semibold text-gray-400 uppercase">Kredit (Rp)</label>
                      <input type="text" value={formatRibuan(row.kredit)} onChange={(e) => handleRowChange(row.id, "kredit", parseRibuan(e.target.value))} onFocus={() => scrollToPreviewField("pv-rincian-perkiraan")} className="w-full p-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-md text-gray-800 dark:text-[#f1f5f3]" />
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white dark:bg-[#0c1410] rounded-2xl border border-gray-100 dark:border-[#213527] p-6 space-y-3 shadow-2xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {Object.keys(checklist).filter((key) => key !== "lainLainText").map((key) => (
                <label key={key} className="flex items-center gap-2 text-xs text-gray-700 dark:text-slate-300 cursor-pointer p-1 rounded-md hover:bg-gray-50 dark:hover:bg-[#1a2b20]/30 transition-colors">
                  <input type="checkbox" checked={checklist[key]} onChange={(e) => setChecklist((prev) => ({ ...prev, [key]: e.target.checked }))} onFocus={() => scrollToPreviewField("pv-kelengkapan-dokumen")} className="rounded text-emerald-600 focus:ring-emerald-500" />
            ...
            {checklist.lainLain && (
              <div className="pt-2">
                <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-1">Nama Dokumen Lainnya</label>
                <input type="text" value={checklist.lainLainText} onChange={(e) => setChecklist((prev) => ({ ...prev, lainLainText: e.target.value }))} onFocus={() => scrollToPreviewField("pv-kelengkapan-dokumen")} className="w-full px-3 py-1.5 text-xs bg-white dark:bg-[#0f1712] border border-gray-200 dark:border-[#2b4533] rounded-lg outline-none text-gray-800 dark:text-[#f1f5f3]" placeholder="Sebutkan..." />
              </div>
            )}
          </div>
        ...
        {/* ================================================================= */}
        {/* PRINTABLE PREVIEW PANEL (RIGHT PANEL) - Letter, koordinat pt exact */}
        {/* ================================================================= */}
        <div id="sopp-preview-scroll-container" className="xl:col-span-7 flex justify-center overflow-x-auto overflow-y-auto w-full max-h-[90vh]">
          <div
            id="sopp-print-area"
            style={{ width: "612pt", height: "792pt", position: "relative", fontFamily: "'Arial Narrow', 'Arial', sans-serif" }}
          ...
          >
            ...
            <div id="pv-nomor-urut" style={{ ...abs(0, 78.4, 612), textAlign: "center", fontSize: "7.8pt", fontWeight: 400 }}>
            ...
            {label(360.8, 225.7, 60, "Cabang:", false, "pv-cabang")}
            ...
            {/* Pajak Ada / Tidak ada */}
            {blackMark(268.2)}
            {label(62.2, 270.8, 40, "Pajak:", false, "pv-pajak")}
            {label(84.1, 270.8, 30, "Ada")}
            {checkbox(147.1, 268.2, 43.5, 12, pajakAda)}
            {label(199.4, 270.8, 40, "Tidak ada")}
            {checkbox(236.5, 268.2, 40.8, 12, !pajakAda)}
            ...
            {label(270.0, 293.4, 40, "Pph Pasal")}
            <div style={fieldBox(324.0, 290.8, 40.4, 12, "center")}>
              <span style={{ fontFamily: "monospace" }}>{pajakAda ? pphPasal : ""}</span>
            </div>
            {label(375.0, 293.4, 30, "Tarif")}
            <div style={fieldBox(405.0, 290.8, 20.0, 12, "center")}>
              <span style={{ fontFamily: "monospace" }}>{pajakAda && tarif ? `${tarif}%` : ""}</span>
            </div>
            {label(438.0, 293.4, 30, "Pajak")}
            <div style={fieldBox(470.0, 290.8, 107.4, 12, "center")}>
              <span style={{ fontFamily: "monospace" }}>{pajakAda ? formatNumberDot(nilaiPajak) : ""}</span>
            </div>

            {/* ============================ TABEL JURNAL ============================ */}
            <div
              id="pv-rincian-perkiraan"
              style={{
                ...abs(TABLE_LEFT, TABLE_TOP, COL_NO + COL_KODE + COL_URAIAN + COL_DEBET + COL_KREDIT),
                border: "0.75pt solid #000",
                boxSizing: "border-box"
              }}
            >
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "7.8pt" }}>
                <colgroup>
                  <col style={{ width: `${COL_NO}pt` }} />
                  <col style={{ width: `${COL_KODE}pt` }} />
                  <col style={{ width: `${COL_URAIAN}pt` }} />
                  <col style={{ width: `${COL_DEBET}pt` }} />
                  <col style={{ width: `${COL_KREDIT}pt` }} />
                </colgroup>
                <thead>
                  <tr style={{ background: "#90c5e3", height: `${HEADER_H}pt` }}>
                    <th style={{ border: "0.75pt solid #000", textAlign: "center", fontWeight: 700 }}>No.</th>
                    <th style={{ border: "0.75pt solid #000", textAlign: "center", fontWeight: 700 }}>Kode Perkiraan</th>
                    <th style={{ border: "0.75pt solid #000", textAlign: "center", fontWeight: 700 }}>Uraian</th>
                    <th style={{ border: "0.75pt solid #000", textAlign: "center", fontWeight: 700 }}>Debet</th>
                    <th style={{ border: "0.75pt solid #000", textAlign: "center", fontWeight: 700 }}>Kredit</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row, idx) => (
                    <tr key={row.id} style={{ height: `${ROW_H}pt` }}>
                      <td style={{ border: "0.75pt solid #000", textAlign: "center" }}>{idx + 1}</td>
                      <td style={{ border: "0.75pt solid #000", textAlign: "left", paddingLeft: "3pt", fontFamily: "monospace" }}>{row.kode}</td>
                      <td style={{ border: "0.75pt solid #000", textAlign: "left", paddingLeft: "3pt" }}>{row.uraian}</td>
                      <td style={{ border: "0.75pt solid #000", textAlign: "right", paddingRight: "3pt", fontFamily: "monospace" }}>
            ...
            {/* ============================ CHECKLIST ============================ */}
            {blackMark(392.3)}
            <div id="pv-kelengkapan-dokumen" style={{ ...abs(62.2, 394.9, 300), fontSize: "7.8pt", fontWeight: 400 }}>
              Kelengkapan Dokumen
            </div>
