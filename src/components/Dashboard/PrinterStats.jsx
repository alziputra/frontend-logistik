  ...
  return (
    <div className="mb-6">
      <div className="flex items-center gap-2 mb-3 pl-1">
        ...
        <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wide">Data Printer (Total: <span className="text-purple-400">{safePrinters.length} Unit</span>)</h3>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            ...
            <div><p className="text-xs text-slate-400 font-medium">Sewa Berjalan</p><p className="text-xl font-bold text-slate-100">{printerStats.berjalan}</p></div>
          </div>
          <div 
            onClick={() => {
          ...
          >
            ...
            <div><p className="text-xs text-slate-400 font-medium">Sewa Habis</p><p className="text-xl font-bold text-slate-100">{printerStats.habis}</p></div>
          </div>
          <div 
            onClick={() => {
          ...
          >
            ...
            <div><p className="text-xs text-slate-400 font-medium">Inventaris Gudang</p><p className="text-xl font-bold text-slate-100">{printerStats.inventaris}</p></div>
          </div>
        </div>
        <div className="lg:col-span-1 bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-4 flex flex-col">
          ...
          <div className="overflow-y-auto custom-scrollbar flex-1 max-h-[88px] pr-1">
            {Object.keys(groupedPrinters).length === 0 ? (
              <p className="text-xs text-slate-500 italic">Belum ada data Printer.</p>
            ) : (
              <ul className="space-y-1.5">
                {Object.entries(groupedPrinters).sort((a,b) => b[1] - a[1]).map(([nama, jumlah]) => (
                  <li key={nama} className="flex justify-between items-center text-xs">
                    ...
                    <span className="font-bold text-purple-400 bg-purple-950/80 px-1.5 py-0.5 rounded border border-purple-800/40 shrink-0">{jumlah}</span>
                  </li>
                ))}
              </ul>
            )}
