...
export default function BangunanTanah({ userRole = "admin", lands = [], landFilter = "", setLandFilter, loadAllData }) {
  ...
  const exportToExcel = () => {
    const rows = filteredLands.map((item, idx) => ({
      ...
      "Tanggal SHGB Mulai": item.tgl_shgb_mulai || item.tgl_mulai_shgb || "",
      "Tanggal SHGB Berakhir": item.tgl_shgb_berakhir || item.tgl_berakhir_shgb || "",
      "Tahun Perolehan": item.tahun_perolehan || "",
      "Luas Lahan (m²)": item.luas_tanah_m2 || item.luas_tanah || "",
      "Luas Pagar (m²)": item.luas_pagar_m2 || item.luas_pagar || "",
      "Luas Bangunan (m²)": item.luas_bangunan_m2 || item.luas_bangunan || "",
      "Keterangan": item.keterangan || "",
    }));

    exportExcelUtil(`Daftar_Tanah_${new Date().toISOString().slice(0, 10)}`, rows);
  };
