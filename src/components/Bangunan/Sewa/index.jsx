...
export default function BangunanSewa({
  ...
  const exportToExcel = () => {
    const rows = filteredSewas.map((item, idx) => ({
      ...
      "Tgl Kontrak Mulai": item.tgl_kontrak_mulai || "",
      "Tgl Kontrak Berakhir": item.tgl_kontrak_berakhir || "",
      "Harga Sewa (Rp)": item.harga_sewa || 0,
      "Alamat": item.alamat || "",
    }));

    exportExcelUtil(`Sewa_Bangunan_${new Date().toISOString().slice(0, 10)}`, rows);
  };
